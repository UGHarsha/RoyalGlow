...
function getServiceDuration(service: string): string {
  const match = service?.match(/(\d+)\s*min/i);
  return match ? `${match[1]} minutes` : '60 minutes';
}

interface BookingEmailHtmlOptions {
  name: string;
  service: string;
...
function buildBookingEmailHtml({ name, service, appointmentDate, siteUrl }: BookingEmailHtmlOptions): string {
...
export async function sendBookingConfirmationEmail({ to, name, service, appointmentDate }: SendBookingEmailParams) {
  ...
  const from = `"Royal Glow Salon" <${process.env.SMTP_USER}>`;
  const formattedDate = formatAppointmentDate(appointmentDate);
  ...
  const logoPath = path.join(process.cwd(), 'public/wite.png');
  const attachments = [];
